...
void mexFunction(int nlhs, mxArray *plhs[],	int nrhs, const mxArray *prhs[]) {
    ...
    double *mhd, *A, *B, min_dist, euclidean_dist;
    int i, j;
    ...
    for(i=0;i<Arows;i++) {
        min_dist = 9999999;
        for(j=0;j<Brows;j++) {
            euclidean_dist = sqrt(((A[i]-B[j])*(A[i]-B[j])) + ((A[Arows+i]-B[Brows+j])*(A[Arows+i]-B[Brows+j])));
    ...
    for(i=0;i<Brows;i++) {
        min_dist = 9999999;
        for(j=0;j<Arows;j++) {
            euclidean_dist = sqrt(((A[j]-B[i])*(A[j]-B[i])) + ((A[Arows+j]-B[Brows+i])*(A[Arows+j]-B[Brows+i])));
            if(euclidean_dist<min_dist)
                min_dist=euclidean_dist;
        }
        rhd += min_dist;
