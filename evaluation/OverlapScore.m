...
function out = OverlapScore(inMask,gtMask)
% calculate overlap score from given masks
...
joint = sum(sum(max(inMask,gtMask))); % cardinality of joint set
overlap= sum(inMask(:).*gtMask(:)); % cardinality of overlap of the two regions
if(joint>eps)
    out = overlap/joint; % overlap score
else
    out = 0;
end